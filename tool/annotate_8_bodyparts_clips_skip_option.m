...
initFolder = '/Volumes/GoogleDrive/My Drive/CRG_Dropbox/AljoComputer/Matlab/high_res_tracker/Data/20180417_BG57_Midpoint_200micron_30fps/RAW/';
% initFolder = '/Volumes/GoogleDrive/My Drive/CRG_Dropbox/AljoComputer/Matlab/high_res_tracker/Data/20180429_BG57_Midpoint_600micron/AVI/';
...
iter = input(prompt);
% expression = '\\MHDO_Tracking'; %% Windows with external HD
% expression = '\MHDO_Tracking'; %% Mac with external HD
expression = '\My Drive'; %% Macbook
...
try
    while j <= parse_number_of_frames & i <= parse_number_of_frames
        ...
        crop_x = max(1, headPosition(j, 1)-150);
        crop_y = max(1, headPosition(j, 2)-150);
        imageShape = size(vid_frame_original);
        axis square
        
        if crop_x+300 > imageShape(1)
            cropIndex_x = crop_x:imageShape(1);
        else
            cropIndex_x = crop_x:crop_x+300;
        end
        if crop_y+300 > imageShape(2)
            cropIndex_y = crop_y:imageShape(2);
        else
            cropIndex_y = crop_y:crop_y+300;
        end
        vid_frame = vid_frame_original(cropIndex_y, cropIndex_x);
        f_image = imshow(vid_frame);
        title(sprintf('Frame # %d', j-startFrame));
    ...
    FileJSON_writer = fopen(fullfile(pathname_tiff,strcat(name,'_',dateName,'_Coordinates.json')),'w');
